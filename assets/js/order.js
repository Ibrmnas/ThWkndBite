(function () {
  ...
  function addRow(tbody, row) {
    ...
    qty.type = 'number'; qty.step = '0.5'; qty.min = '0.5'; tdQty.appendChild(qty);
  ...
  async function sendToSheet() {
    ...
    // Validate minimums: total >= 1kg; per line either 0 or >= 0.5 in 0.5 steps
...
// Allergies: single text field (optional)
const allergiesText = (document.getElementById('c-allergies')?.value || "").trim();
if (allergiesText) {
  const tag = `[Allergies: ${allergiesText}]`;
  payload.notes = payload.notes ? (payload.notes + ' ' + tag) : tag;
  payload.allergies_text = allergiesText;
}
